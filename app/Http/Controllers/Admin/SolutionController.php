...
use App\Model\Admin\Solution as ThisModel;
...
{
    protected $view = 'admin.solutions';
    protected $route = 'solutions';
    ...
    public function nestedSort(Request $request)
    {
        if ($request->ajax()) {
            ...
            foreach($readbleArray as $row){
                ...
                $i++;
                DB::table('solutions')->where('id',$row['id'])->update(['sort_order' => $i]);
            }
            $json->success = true;
    ...
    {
        $category = Solution::query()->find($request->cate_id);
