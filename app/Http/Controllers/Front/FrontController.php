...
class FrontController extends Controller
{
    ...
    public function solutions(Request $request, $slug)
    {
        $solution = Solution::query()->with(['image', 'banner'])->where('slug', $slug)->first();
        $products = Product::query()->with('image')
            ->where('solution_id', $solution->id)->latest()->paginate(15);
        $solutions = Solution::query()->orderBy('sort_order')->get();

        return view('site.solutions', compact('solution', 'products', 'solutions'));
    }
    public function homePage() {
        $data['banners'] = Banner::query()->with('image')->latest()->get();
        $data['categories'] = Category::query()->where('show_home_page',1)
            ->where('parent_id', 0)
            ->with('image')->orderBy('sort_order')->get();
        $data['solutions'] = Solution::query()->where('show_home_page',1)
            ->where('parent_id', 0)
            ->with('image')->orderBy('sort_order')->get();
        $data['posts'] = Post::query()
            ->where('status', 1)
            ->with('image')->latest()->get()->take(4);
    ...
    public function getProductDetail($slug) {
        $product = Product::query()->with(['category', 'image', 'galleries.image'])->where('slug', $slug)->firstOrFail();
    ...
    public function searchProducts(Request $request) {
        ...
        if($request->cate) {
            if($request->cate == 'all') {
                ...
                $products = $products->where('cate_id', $request->cate);
            }
        }
        ...
        return view('site.search', compact('products', 'keyword', 'allCates'));
    }
