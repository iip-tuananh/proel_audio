...
@endsection
@section('buttons')
<a href="{{ route('solutions.create') }}" class="btn btn-outline-success btn-sm" class="btn btn-info"><i class="fa fa-plus"></i> Thêm mới</a>
                ...
                <div class="dd" id="nestable">
                  ...
                  $html = "<ol class=\"".$class."\" id=\"menu-id\">";
                    foreach($items as $key=>$value) {
                          $icon = $value['show_home_page']
        ? '<i class="fas fa-eye text-success mx-1" title="Hiển thị trang chủ"></i>'
        : '<i class="fas fa-eye-slash text-muted mx-1" title="Không hiển thị"></i>';
                    ...
                    $html.= '<li class="dd-item dd3-item" data-id="'.$value['id'].'">
                      ...
                      '.$icon.'
                        <span class="span-right">
                          ...
                          <a class="edit-button" href="'. route('solutions.edit',$value['id']) .'" id="'.$value['id'].'" label="'.$value['name'].'"><i class="fas fa-pencil-alt"></i></a>
                          <a class="del-button confirm"  href="'. route('solutions.delete',$value['id']) .'" id="'.$value['id'].'"><i class="fa fa-trash"></i></a></span>
                      </div>';
...
<script>
  $(document).ready(function(){
    ...
    $('.dd').on('change', function() {
      ...
      $.ajax({
        type: "POST",
        url: baseURL + '/admin/solutions/nested-sort',
        data: {dataString},
        cache : false,
        headers: {
  ...
  app.controller('Category', function ($rootScope, $scope, $http) {
      ...
      $('.link-button').on('click', function (e) {
          ...
          $.ajax({
              type: 'GET',
              url: "/admin/solutions/" + cate_id + "/getDataForEdit",
              headers: {
                  'X-CSRF-TOKEN': CSRF_TOKEN
              },
              success: function (response) {
      ...
      $scope.submit = function () {
          ...
          $.ajax({
              type: 'POST',
              url: "{!! route('solutions.add.homepage') !!}",
              headers: {
                  'X-CSRF-TOKEN': CSRF_TOKEN
              },
              data:{
