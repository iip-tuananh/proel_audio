...
@section('content')
    <div data-elementor-type="product" data-elementor-id="1469"
        ...
        <div class="elementor-element elementor-element-443fe0db e-flex e-con-boxed e-con e-parent" data-id="443fe0db"
             data-element_type="container" data-settings="{&quot;background_background&quot;:&quot;classic&quot;}">
            <div class="e-con-inner">
                <div class="elementor-element elementor-element-6a0d58a6 elementor-widget__width-inherit elementor-widget elementor-widget-spacer"
                     data-id="6a0d58a6" data-element_type="widget" data-widget_type="spacer.default">
                ...
                <div class="elementor-element elementor-element-287596ad elementor-widget elementor-widget-woocommerce-breadcrumb"
                     data-id="287596ad" data-element_type="widget" data-widget_type="woocommerce-breadcrumb.default">
                    <div class="elementor-widget-container">
                        <nav class="woocommerce-breadcrumb" aria-label="Breadcrumb"><a href="{{ route('front.home-page') }}">Trang chủ</a>&nbsp;&#47;&nbsp;<a
                                href="{{ route('front.getListProduct', $product->category->slug) }}">{{ $product->category->name ?? '' }}</a>&nbsp;&#47;&nbsp;{{$product->name}}
                        </nav>
                    </div>
                </div>
            </div>
        </div>
        <div class="elementor-element elementor-element-9bb4264 schedaPDF1 e-flex e-con-boxed e-con e-parent"
             data-id="9bb4264" data-element_type="container">
            <div class="e-con-inner">
                <div class="elementor-element elementor-element-1310423 PDFA e-con-full e-flex e-con e-child"
                     data-id="1310423" data-element_type="container">
                    <div class="elementor-element elementor-element-fd18204 elementor-widget elementor-widget-woocommerce-product-images"
                         data-id="fd18204" data-element_type="widget" data-widget_type="woocommerce-product-images.default">
                        <div class="elementor-widget-container">
                            <div class="woocommerce-product-gallery woocommerce-product-gallery--with-images woocommerce-product-gallery--columns-4 images"
                                 data-columns="4" style="opacity: 0; transition: opacity .25s ease-in-out;">
                                <div class="woocommerce-product-gallery__wrapper">
                                    <div data-thumb="{{ $product->image->path ?? '' }}"
                                         data-thumb-alt=""
                                         data-thumb-srcset="{{ $product->image->path ?? '' }} 100w,
                                            {{ $product->image->path ?? '' }} 300w,
                                             {{ $product->image->path ?? '' }} 1024w,
                                             {{ $product->image->path ?? '' }} 150w,
                                             {{ $product->image->path ?? '' }} 768w,
                                             {{ $product->image->path ?? '' }} 600w,
                                             {{ $product->image->path ?? '' }} 1200w"
                                         ...
                                         class="woocommerce-product-gallery__image"><a
                                            href="{{ $product->image->path ?? '' }}"><img
                                                ...
                                                data-large_image="{{ $product->image->path ?? '' }}"
                                                data-large_image_width="1200" data-large_image_height="1200" decoding="async"
                                                srcset="{{ $product->image->path ?? '' }} 600w,
                                                     {{ $product->image->path ?? '' }} 300w,
                                                      {{ $product->image->path ?? '' }} 1024w,
                                                     {{ $product->image->path ?? '' }} 150w,
                                                     {{ $product->image->path ?? '' }} 768w,
                                                      {{ $product->image->path ?? '' }} 100w,
                                                       {{ $product->image->path ?? '' }} 1200w"
                                                sizes="(max-width: 600px) 100vw, 600px"/></a>
                                    </div>

                                    @if(! $product->galleries->count())
                                        <div data-thumb="{{ $gallery->image->path ?? '' }}"
                                             data-thumb-alt=""
                                             data-thumb-srcset="{{ $gallery->image->path ?? '' }} 100w,
                                            {{ $gallery->image->path ?? '' }} 300w,
                                             {{ $gallery->image->path ?? '' }} 1024w,
                                             {{ $gallery->image->path ?? '' }} 150w,
                                             {{ $gallery->image->path ?? '' }} 768w,
                                             {{ $gallery->image->path ?? '' }} 600w,
                                             {{ $gallery->image->path ?? '' }} 1200w"
                                             data-thumb-sizes="(max-width: 100px) 100vw, 100px"
                                             class="woocommerce-product-gallery__image"><a
                                                href="{{ $gallery->image->path ?? '' }}"><img
                                                    fetchpriority="high" width="600" height="600"
                                                    src="{{ $gallery->image->path ?? '' }}"
                                                    class="wp-post-image" alt="{{ $product->name }}"
                                                    data-caption=""
                                                    data-src="{{ $gallery->image->path ?? '' }}"
                                                    data-large_image="{{ $gallery->image->path ?? '' }}"
                                                    data-large_image_width="1200" data-large_image_height="1200" decoding="async"
                                                    srcset="{{ $gallery->image->path ?? '' }} 600w,
                                                     {{ $gallery->image->path ?? '' }} 300w,
                                                      {{ $gallery->image->path ?? '' }} 1024w,
                                                     {{ $gallery->image->path ?? '' }} 150w,
                                                     {{ $gallery->image->path ?? '' }} 768w,
                                                      {{ $gallery->image->path ?? '' }} 100w,
                                                       {{ $gallery->image->path ?? '' }} 1200w"
                ...
                <div class="elementor-element elementor-element-74dd1c1 PDFB e-con-full e-flex e-con e-child"
                     data-id="74dd1c1" data-element_type="container">
                    <!-- dce invisible element fff4922 --><!-- dce invisible element c704d85 -->
                    <div class="elementor-element elementor-element-48f7e42 elementor-widget elementor-widget-heading"
                         data-id="48f7e42" data-element_type="widget" data-widget_type="heading.default">
                        <div class="elementor-widget-container">
                            <h2 class="elementor-heading-title elementor-size-default">{{ $product->name }}</h2></div>
                    </div>
{{--                    <div class="elementor-element elementor-element-5690e97 elementor-widget elementor-widget-woocommerce-product-title elementor-page-title elementor-widget-heading"--}}
{{--                         data-id="5690e97" data-element_type="widget" data-widget_type="woocommerce-product-title.default">--}}
{{--                        <div class="elementor-widget-container">--}}
{{--                            <h1 class="product_title entry-title elementor-heading-title elementor-size-default">1000W--}}
{{--                                PROCESSED 12&#8243; ACTIVE SPEAKER</h1></div>--}}
{{--                    </div>--}}
                    <div class="elementor-element elementor-element-5c04228 elementor-widget elementor-widget-text-editor"
                         data-id="5c04228" data-element_type="widget" data-widget_type="text-editor.default">
                        <div class="elementor-widget-container">
                            {!! $product->intro !!}
                        </div>
                    </div>
                    <div class="elementor-element elementor-element-34ed68e elementor-align-left elementor-widget elementor-widget-button"
                         data-id="34ed68e" data-element_type="widget"
                ...
                <div class="elementor-column elementor-col-50 elementor-top-column elementor-element elementor-element-319b1c6c PDFC"
                    ...
                    <div class="elementor-widget-wrap elementor-element-populated">
                        <div class="elementor-element elementor-element-52c8bef5 elementor-widget elementor-widget-heading"
                            ...
                            <div class="elementor-widget-container">
                                <h2 class="elementor-heading-title elementor-size-default">Thông số sản phẩm</h2>
                            </div>
                        </div>
                                ...
                                <div class="elementor-main-swiper swiper-container swiper">
                                    <div class="ecs-posts elementor-posts-container elementor-posts swiper-wrapper   elementor-posts--skin-custom"
                                        ...
                                        @foreach($otherProducts as $otherProduct)
                                            <article id="post-{{ $otherProduct->id }}"
                                                            ...
                                                            <div class="elementor-column elementor-col-100 elementor-top-column elementor-element elementor-element-2b043617"
                                                                ...
                                                                <div class="elementor-widget-wrap elementor-element-populated">
                                                                    <div class="elementor-element elementor-element-14ef3f39 elementor-widget elementor-widget-image"
                                                                         ...
                                                                         data-widget_type="image.default">
                                                                        <div class="elementor-widget-container">
                                                                            <a href="{{ route('front.getProductDetail', $otherProduct->slug) }}">
                                                                                <img src="{{ $otherProduct->image->path ?? '' }}"
                                                                                     title="{{ $otherProduct->name }}" alt="{{ $otherProduct->name }}"
                                                                                     loading="lazy"/> </a>
                                                                        </div>
                                                                    </div>
